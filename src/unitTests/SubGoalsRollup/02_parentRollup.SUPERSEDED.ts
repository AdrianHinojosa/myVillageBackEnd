 ...
 *
 * It is NOT registered in `run.ts`, so `npm run test:subgoals` does not execute it.
 *
 * Every line below is prefixed with a line comment rather than wrapped in a block comment, and that
 * detail matters: `npm run build` runs babel over ALL of src/, and block comments do not nest — the
 * first closing delimiter inside the file ends the wrapper early and leaves the remainder as invalid
 * code. That is exactly how this file broke the build once. Line comments cannot do that.
 *
 * To resurrect it, the averaging branch in `recalculateParentRollup()` would have to come back too —
 * see the commented-out block in `trackingRecords.queries.ts`.
 */
