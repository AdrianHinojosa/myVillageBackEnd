...
| Front-End Field | Back-End Field | Notes |
|---|---|---|
| `sBirthYear` (string in form) | `iBirthYear` (integer) | Front-end must parse to integer before sending |
...
`aDocuments` sent in create/update goal body is **ignored** by the back-end. File uploads must be handled separately via the GoalFiles routes (`POST /goals/:sGoalId/goalFiles`). Remove `aDocuments` from the goals form submission payload.
